...
from dynamic_functions.Data.main import game_dir, get_player_position
...
def get_role_for_bot(game_id: str, bot_sid: str, user_sid: Optional[str] = None) -> Optional[Dict[str, Any]]:
    """Look up a bot's assigned role from the game's characters.json.

    Returns a dict with id, title, location, bot, requiresCheckin —
    the shape that chat_callback and BotChatContext expect.
    """
    characters = _load_characters()
    for ch in characters:
        if ch.get("sid") == bot_sid:
            ...
            game_dir_path = _find_game_dir()
            role_dir = os.path.join(game_dir_path, "Roles", role_name)

            caller = user_sid or atlantis.get_caller()
            location = ""
            if game_id and caller:
                location = get_player_position(game_id, caller) or ""
            ...
            return {
                ...
                "location": location,
                "bot": bot_sid,
                "requiresCheckin": os.path.isfile(os.path.join(role_dir, "checkin.py")) if os.path.isdir(role_dir) else False,
            }
    return None
