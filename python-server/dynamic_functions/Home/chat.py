...
async def chat():
    ...
    if True:
        # Define the assistant base prompt
...
You are a helpful professional assistant for FlowCentral, an enterprise automation platform.
You help users build, share, and manage automation tools using the Model Context Protocol (MCP).
You are knowledgeable, efficient, and friendly. You communicate clearly and professionally.
To help you assist people, you may have some tools available.
New users can type '/help' to see available commands.
"""
        }
        ...
        if last_chat_entry and last_chat_entry.get('type') == 'chat' and last_chat_entry.get('sid') == 'assistant':
            logger.warning("\x1b[38;5;204mLast chat entry was from assistant (bot), skipping response\x1b[0m")
            await atlantis.owner_log(f"Skipping response - last chat was from assistant (sid={last_chat_entry.get('sid')})")
            return
        ...
        # Check if first message is system message, if not add assistant system message at front
        if rawTranscript[0].get('role') != 'system':
            rawTranscript.insert(0, ASSISTANT_SYSTEM_MESSAGE)
            logger.info("Inserted assistant system message at front of transcript")
        else:
            # Replace existing system message with assistant's system message
            logger.warning(f"!!! Replacing existing system message. Old: {rawTranscript[0].get('content', '')[:100]}...")
            rawTranscript[0] = ASSISTANT_SYSTEM_MESSAGE
        ...
        transcript.append({'role': 'system', 'content': ASSISTANT_SYSTEM_MESSAGE['content']})
        logger.info(f"  [sys] Injected assistant system message ({len(ASSISTANT_SYSTEM_MESSAGE['content'])} chars)")
        ...
        for i, msg in enumerate(rawTranscript):
            ...
            if msg_type == 'chat':
                ...
                # - everyone else = user
                role_for_llm = 'assistant' if msg_sid == 'assistant' else 'user'
        ...
        try:
            ...
            streamTalkId = await atlantis.stream_start("assistant","Assistant")
