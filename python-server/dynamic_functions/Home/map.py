...
async def map(location: str = "") -> None:
    ...
    def _node_html(node: dict) -> str:
        ...
        if node["image"]:
            ...
            img_html = f'<div class="map-img-placeholder-{uid}">\U0001f5fa\ufe0f</div>'

        # Characters present
        chars_html = ""
        if node["characters"]:
            chars_list = ", ".join(_esc(c) for c in node["characters"])
            chars_html = f'<div class="map-chars-{uid}">\U0001f464 {chars_list}</div>'

        # Label
        label = _esc(node["description"])
        if is_current:
            label = f"\U0001f4cd {label}"
...
"""
    ...
    await atlantis.client_log(f"\U0001f4cd Map centered on: {center_loc.get('description', location)}")
