...
@chat
async def atlas():
    """Main chat function for FlowCentral"""
    ...
    if True:
        # Define the Atlas professional prompt
        ATLAS_SYSTEM_MESSAGE = {
            "type": "chat",
            "sid": "system",
            "role": "system",
            "content": """
...
"""
        ...
        # Check if first message is system message, if not add Atlas system message at front
        if rawTranscript[0].get('role') != 'system':
            rawTranscript.insert(0, ATLAS_SYSTEM_MESSAGE)
            logger.info("Inserted Atlas system message at front of transcript")
        else:
            # Replace existing system message with Atlas system message
            logger.warning(f"!!! Replacing existing system message. Old: {rawTranscript[0].get('content', '')[:100]}...")
            rawTranscript[0] = ATLAS_SYSTEM_MESSAGE
        ...
        transcript.append({'role': 'system', 'content': ATLAS_SYSTEM_MESSAGE['content']})
        logger.info(f"  [sys] Injected Atlas system message ({len(ATLAS_SYSTEM_MESSAGE['content'])} chars)")
